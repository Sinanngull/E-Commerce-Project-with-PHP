...
<head>
<meta charset="utf-8">
<title> Bilshop Yönetim Paneli - Ürün Ekle </title>
...
<?php include_once ("inc-panel/aside.php"); ?>
        ...
        <div id="page_content">
            ...
            if ( isset($_POST["btn_ekle"]) ) {
                ...
                // Resmin Uzantısını ve Boyutunu kontrol ettir.
                if (  $uzanti != "jpg" and $uzanti != "jpeg" and $uzanti != "gif" and $uzanti != "png" ) {
                    ...
                    // Ürün Ekleme işlemi başladı

                    // 1-) Resim Adını SEO Uyumlu hale getir
                    $yeni_resim_adi     = seo_url($urun_adi).".".$uzanti;

                    // 2-) Ekleme için SQL Sorgusu
                    $ekle = $veritabani -> prepare ("insert into urunler set kategori_id=:kategori_id, sira_no=:sira_no, urun_adi=:urun_adi, description=:description, resim=:resim, urun_aciklama=:urun_aciklama, fiyat=:fiyat, kdv_oran=:kdv_oran, stok_miktari=:stok_miktari, durum=:durum");
                    $sonuc = $ekle -> execute ( array("kategori_id"=>$kategori_id, "sira_no"=>$sira_no, "urun_adi"=>$urun_adi, "description"=>$description, "resim"=>$yeni_resim_adi, "urun_aciklama"=>$urun_aciklama, "fiyat"=>$fiyat, "kdv_oran"=>$kdv_oran, "stok_miktari"=>$stok_miktari, "durum"=>$durum ) );


                     if ( $sonuc ) {
                        echo "Ürün Eklendi <br>";
                        // 3-) Ürün Eklendiği İçin Resmi Upload Et
                        ...
                        echo "HATA : Ürüm Eklenemedi";
                     }
                     

                    // Ürün Ekleme işlemi bitti
                }
